...
y = 1000L  #if we add L in end of any number it is consider as a long integer
...
1 != 6 # not equal to (!= is consider as  not equal to)
...
#Homogeneous vectors (same character or integer)
#vector of strings

fruits = c("Banana","Apple","Orange","Strawberry")
class(fruits)  
names=c("harish","tharun","vaishu")
names
#vector of numerical
n1 = c(1,2,3,4)
class(n1)
...
df1$Training #$it is used to get the exact data
...
# Rows with food type: Veg [use indexing to solve]
...
dim(Orange) # dim is dimentions of dataframe
dim(food)
dim(df1)
